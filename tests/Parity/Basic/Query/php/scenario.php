<?php

declare(strict_types=1);

namespace Temporal\Tests\Parity\Basic\Query\Php;

use Carbon\CarbonInterval;
use Temporal\Client\WorkflowClientInterface;
use Temporal\Client\WorkflowOptions;
use Temporal\Worker\WorkerInterface;
use Temporal\Workflow;
use Temporal\Workflow\QueryMethod;
use Temporal\Workflow\SignalMethod;
use Temporal\Workflow\WorkflowInterface;
use Temporal\Workflow\WorkflowMethod;

#[WorkflowInterface]
final class QueryWorkflow
{
    private int $counter = 0;
    private bool $done = false;

    #[WorkflowMethod(name: 'Parity_Basic_Query')]
    public function run()
    {
        yield Workflow::await(fn(): bool => $this->done);

        return "final:{$this->counter}";
    }

    #[QueryMethod(name: 'get_counter')]
    public function getCounter(): int
    {
        return $this->counter;
    }

    #[SignalMethod(name: 'bump')]
    public function bump(): void
    {
        ++$this->counter;
    }

    #[SignalMethod(name: 'finish')]
    public function finish(): void
    {
        $this->done = true;
    }
}

function parity_php_register(WorkerInterface $worker): void
{
    $worker->registerWorkflowTypes(QueryWorkflow::class);
}

function parity_php_run(WorkflowClientInterface $client, string $taskQueue): string
{
    $workflowId = 'parity-basic-query-php-' . \bin2hex(\random_bytes(6));

    $stub = $client->newUntypedWorkflowStub(
        'Parity_Basic_Query',
        WorkflowOptions::new()
            ->withWorkflowId($workflowId)
            ->withTaskQueue($taskQueue)
            ->withWorkflowExecutionTimeout(CarbonInterval::seconds(15)),
    );

    $client->start($stub);
    \usleep(150_000);
    $stub->signal('bump');
    \usleep(150_000);
    $stub->signal('bump');
    \usleep(150_000);
    $counter = $stub->query('get_counter')?->getValue(0);
    if ($counter !== 2) {
        throw new \RuntimeException("unexpected counter: " . \var_export($counter, true));
    }
    $stub->signal('finish');

    $result = $stub->getResult('string');
    if ($result !== 'final:2') {
        throw new \RuntimeException("unexpected: " . \var_export($result, true));
    }

    return $workflowId;
}
