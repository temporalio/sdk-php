...
final class Process
{
     ...
     */
    public function __construct(TaskQueueInterface $worker, WorkflowContext $ctx, WorkflowInstance $instance)
    {
        $this->worker = $worker;
        $this->context = $ctx;
    ...
    private function nextPromise(PromiseInterface $promise): void
    {
        $onFulfilled = function ($result) {
            $this->worker->once(TaskQueueInterface::ON_TICK, function () use ($result) {
                Workflow::setCurrentContext($this->getContext());
        ...
        $onRejected = function (\Throwable $e) {
            $this->worker->once(TaskQueueInterface::ON_TICK, function () use ($e) {
                Workflow::setCurrentContext($this->getContext());
