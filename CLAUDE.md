...
- A `#[Service]`-annotated **interface** declares operations as plain method
  ...
  `#[AsyncOperation(output: '<wire-type>')]` and return `OperationInfo`.
...
- Async ops must be paired with a method carrying
  `#[OperationCancel(operation: 'opName')]` that takes the operation token as
  its sole argument. The factory binds it as the cancel routine; `ServiceHandler`
  rejects cancel requests for sync ops with `ErrorType::NotImplemented`.
- Inside an impl method body, reach the surrounding dispatch state via the
  static `Nexus::` accessors:
  - `Nexus::getCurrentContext()` — handler-side `OperationContext` (links,
    headers, deadline)
...
`WorkflowRunOperation` is a static helper called inline from an
...
The previous handler-factory shape (`#[ServiceImpl]`, `#[OperationImpl]`,
methods returning `OperationHandlerInterface` via `SynchronousOperationHandler`)
is gone — those attributes and classes have been deleted. `OperationHandlerInterface`
itself remains as the internal adapter contract (used by middlewares); user
code never implements it directly.
