...
| [Интеграция](integration.md) | Где `DataConverter` создаётся и инжектится, поздняя инъекция через `setDataConverter`, codec wire-boundary, таблица routes, Nexus payload, failures, [serialization context](integration.md#serialization-context) |
