...
Converter может узнать, какому workflow/activity принадлежит payload — например, чтобы подписать его `workflowId` (защита от payload-replay) или вывести ключ шифрования из namespace. Это паритет с Java (`sdk-java#1695`), Go (`sdk-go#1352`) и TypeScript.

Контекст моделируется в публичном namespace `Temporal\DataConverter\`:

| Тип | Поля |
|---|---|
| [`WorkflowSerializationContext`](../../src/DataConverter/WorkflowSerializationContext.php) | `namespace`, `workflowId` |
| [`ActivitySerializationContext`](../../src/DataConverter/ActivitySerializationContext.php) | `namespace`, `workflowId?`, `workflowType?`, `activityType?`, `taskQueue?`, `isLocal` |

Оба реализуют [`HasWorkflowSerializationContext`](../../src/DataConverter/HasWorkflowSerializationContext.php) (`getNamespace()`, `getWorkflowId()`), который расширяет marker [`SerializationContext`](../../src/DataConverter/SerializationContext.php).

**Механизм (opt-in, без BC-слома).** PHP-интерфейсы не умеют default-методы, поэтому `DataConverterInterface`/`PayloadConverterInterface` НЕ меняются. Converter, которому нужен контекст, дополнительно реализует [`SerializationContextAwareInterface`](../../src/DataConverter/SerializationContextAwareInterface.php):

```php
public function withSerializationContext(?SerializationContext $context): static;
```

Метод возвращает **новый** (immutable) экземпляр, привязанный к контексту. Сторонние converter'ы, которые его не реализуют, продолжают работать без изменений и контекста не получают (паритет с Go). Привязка делается через [`SerializationContextBinder::bind()`](../../src/DataConverter/SerializationContextBinder.php) — он отдаёт converter как есть, если контекст `null` или converter не context-aware.

**Где контекст хранится.** Контекст едет вместе с данными: [`EncodedValues`](../../src/DataConverter/EncodedValues.php) держит `?SerializationContext` (`setSerializationContext()`/`getSerializationContext()`) и применяет его к своему converter'у **в момент конвертации** (`getValue()`/`toPayloads()`), вызывая `SerializationContextBinder::bind()`. Поэтому codec [`Encoder`](../../src/Worker/Transport/Codec/JsonCodec/Encoder.php) по-прежнему ставит базовый converter через `setDataConverter()` — контекст лежит в отдельном поле и не затирается. Глобально на shared converter контекст ставить нельзя: один converter обслуживает много payload'ов с разными контекстами, а codec флашит батч команд разом.

**Какой контекст где привязывается:**

| Сценарий | Контекст | Где |
|---|---|---|
| Client outbound (start/signal/query/update + result) | `WorkflowSerializationContext` | [`WorkflowStarter`](../../src/Internal/Client/WorkflowStarter.php), [`WorkflowStub`](../../src/Internal/Client/WorkflowStub.php) |
| Workflow input/output, side-effect, signal/query/update | `WorkflowSerializationContext` | [`StartWorkflow`](../../src/Internal/Transport/Router/StartWorkflow.php), [`WorkflowContext`](../../src/Internal/Workflow/WorkflowContext.php), Invoke{Signal,Query,Update} |
| Activity input/output/heartbeat | `ActivitySerializationContext` | [`InvokeActivity`](../../src/Internal/Transport/Router/InvokeActivity.php) (+ `InvokeLocalActivity` для `isLocal`), [`ActivityContext`](../../src/Internal/Activity/ActivityContext.php) |
| Workflow-issued activity / child / external | activity- или workflow-context | [`ActivityStub`](../../src/Internal/Workflow/ActivityStub.php), [`ChildWorkflowStub`](../../src/Internal/Workflow/ChildWorkflowStub.php), [`ExternalWorkflowStub`](../../src/Internal/Workflow/ExternalWorkflowStub.php) |
| Результат outbound-команды (activity/child) | контекст запроса | [`Client::dispatch`](../../src/Internal/Transport/Client.php) перевязывает response payload'ы контекстом, записанным при отправке |
| Async activity completion, Schedule | activity-/workflow-context | [`ActivityCompletionClient`](../../src/Internal/Client/ActivityCompletionClient.php), [`ScheduleMapper`](../../src/Internal/Mapper/ScheduleMapper.php) |
| Детали падений (failure) | контекст соответствующего scope | `TemporalFailure::setSerializationContext()` навешивает контекст на `details`/`lastHeartbeatDetails` по всей цепочке `getPrevious()`; вызывается на client-decode (workflow-failed, update-failure), worker-decode (activity/child через `Client::dispatch`) и worker-encode (`InvokeActivity`, `InvokeUpdate`) |

**Падения (failure paths).** `details` у `ApplicationFailure`/`CanceledFailure`/`TimeoutFailure` подписываются тем же контекстом, что и успешный путь: воркер кодирует детали workflow-/activity-контекстом, а клиент/воркер декодируют тем же. Механизм — метод `setSerializationContext(?SerializationContext)` в иерархии `TemporalFailure` (зеркало существующего `setDataConverter()`), который рекурсивно проходит причинно-следственную цепочку. Без этого context-aware converter (подпись/шифрование по `workflowId`) ломался бы на любом падении workflow/update/activity.

**Намеренно без контекста:** header'ы, `Memo`, `SearchAttributes` (паритет с Java — их не подписывают; `Memo`/`SearchAttributes` в PHP вообще не проходят через converter, они едут сырыми в `options`).

**Известные ограничения:** авто-генерируемый child `workflowId` неизвестен на момент сериализации (генерится сервером), поэтому такой child получает контекст с `null` workflowId; completion по task-token знает только токен (контекст почти пустой).
