...
final class MyService
{
    ...
    public function longJob(string $input): OperationInfo
    {
        return WorkflowRunOperation::start(
            WorkflowHandle::fromWorkflowMethod(MyWorkflow::class, $opts, $input),
            Nexus::getStartDetails(),
        );
    }

    #[OperationCancel(operation: 'longJob')]
    public function cancelLongJob(string $token): void
    {
...
> Это новая модель (после удаления `#[ServiceImpl]` / `#[OperationImpl]` / `SynchronousOperationHandler` factory'ев). Старые атрибуты в проекте удалены.
...
|---|---|
| `Nexus::getCurrentContext()` | `OperationContext` (handler-side: links, headers, deadline) |
...
`getOperationContext()` пробрасывает `WorkflowClient` из `NexusEnvironment`, создаваемого воркером — handler через него может стартовать workflow, signal'ить и т.д.
...
Для метода с `#[AsyncOperation]` тип возврата — `OperationInfo` (а не `OperationStartResult`); SDK сам обернёт его в `AsyncOperationStartResult` через `WorkflowRunOperation::start()`.
...
- `isMethodCancelled()` — это RR-уровневая отмена (deadline, shutdown, остановка handler'а воркером), **не** Nexus-spec'овая отмена операции (для второй есть отдельный путь через `#[OperationCancel]`).
...
1. Резолв `(service, operation) → ServiceImplInstance + OperationHandlerInterface` через таблицу инстансов. Если service неизвестен — `HandlerException(NotFound)` (`'Unrecognized service ...'`). Если операция неизвестна — `HandlerException(NotFound)`.
...
6. **Sync-операции не отменяемы**: если `cancelOperation` пришёл на sync-операцию (handler — `SynchronousOperationHandler`), бросает `HandlerException(NotImplemented)` с сообщением `'Operation %s/%s is synchronous and cannot be cancelled'`.
...
[src/Internal/Nexus/NexusEnvironment.php](../../src/Internal/Nexus/NexusEnvironment.php):

```php
final class NexusEnvironment
{
    public function __construct(
        public readonly string $namespace,           // non-empty
        public readonly string $taskQueue,           // non-empty
        public readonly WorkflowClientInterface $workflowClient,
    ) { /* validation */ }
}
```

Создаётся в [src/WorkerFactory.php:200-203](../../src/WorkerFactory.php:200) и пробрасывается в `Worker` ([src/Worker/Worker.php:42](../../src/Worker/Worker.php:42)). Через него handler-методы получают `WorkflowClient` для запуска workflow'ов и cross-workflow операций. Без `NexusEnvironment` `Nexus::getOperationContext()` вернуть Temporal-side контекст не сможет — handler сможет работать только в рамках чистой Nexus-семантики (sync-операции без обращения к workflow API).
