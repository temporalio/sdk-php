...
Это spec'овый cancel-by-token. RR получил `POST /{service}/{operation}/cancel?token=...` от внешнего caller'а и пробросил его в PHP. PHP вызывает user-defined `#[OperationCancel]`-метод.
