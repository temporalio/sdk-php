...
| [Handler-side SDK](handler-side-sdk.md) | `#[Service]`, `#[Operation]`, `#[AsyncOperation]`, `Nexus::` accessors, `ServiceHandler`, `NexusEnvironment` |
...
- `~/IdeaProjects/temporalio/nexus-rpc-sdk-php` — handler-side SDK для PHP (подключается через composer `path` repo).
